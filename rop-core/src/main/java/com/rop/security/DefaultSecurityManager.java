...
public class DefaultSecurityManager implements SecurityManager {
    ...
    public MainError validateSystemParameters(RopRequestContext context) {
        ...
        //2.检查session_token
        mainError = checkSessionToken(context);
        if (mainError != null){
            return mainError;
        }

        //3.检查会话
        mainError = checkAccessToken(context);
        if (mainError != null) {
            return mainError;
        }
    ...
    private MainError checkInvokeTimesLimit(RopRequestContext rrctx) {
        ...
        } else if (invokeTimesController.isSessionInvokeLimitExceed(rrctx.getAppKey(), rrctx.getAccessToken())) {
     ...
     * @return
     */
    private MainError checkSessionToken(RopRequestContext context) {
        if (context.getServiceMethodHandler() != null &&
                context.getServiceMethodHandler().getServiceMethodDefinition().isNeedSessionToken()) {
            if (context.getSessionToken() == null) {
                return MainErrors.getError(MainErrorType.MISSING_SESSION_TOKEN, context.getLocale(),
                        context.getMethod(), context.getVersion(), SystemParameterNames.getSessionToken());
            } else {
                if (!isValidSessionToken(context)) {
                    return MainErrors.getError(MainErrorType.INVALID_SESSION_TOKEN, context.getLocale(),
                            context.getMethod(), context.getVersion(),context.getSessionToken());
                }
            }
        }
        return null;
     ...
     * @return
     */
    private MainError checkAccessToken(RopRequestContext context) {
        //需要进行session检查
        if (context.getServiceMethodHandler() != null &&
            ...
            if (context.getAccessToken() == null) {
                return MainErrors.getError(MainErrorType.MISSING_ACCESS_TOKEN, context.getLocale(),
                        context.getMethod(), context.getVersion(), SystemParameterNames.getAccessToken());
            } else {
                if (!isValidAccessToken(context)) {
                    return MainErrors.getError(MainErrorType.INVALID_ACCESS_TOKEN, context.getLocale(),
                            context.getMethod(), context.getVersion(),context.getAccessToken());
                }
            }
        }
        return null;
    ...
    private boolean isValidSessionToken(RopRequestContext smc){
        if(!sessionManager.hasSession(smc.getSessionToken())){
            if (logger.isDebugEnabled()) {
                logger.info(smc.getAccessToken() + "session_token过期");
            }
            return false;
        } else {
            return true;
    ...
    private boolean isValidAccessToken(RopRequestContext smc) {
        if (!sessionManager.hasSession(smc.getAccessToken())) {
            if (logger.isDebugEnabled()) {
                logger.info(smc.getAccessToken() + "access_token过期");
            }
            return false;
        } else {
            return true;
